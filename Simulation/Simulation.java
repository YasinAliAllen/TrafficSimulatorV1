...
public class Simulation {
    int speedMax, time;
    private final int MAXSPEED = 3, STOPTIME = 50, NUMROADS = 2, NUMCARS = 1;
            ...
            for (int i = 0; i < roads.size(); i++) { //moves any vehicle on a road
                if (roads.get(i).getVehicle() != null) { //checks for vehicle
                    if (roads.get(i).getVehicle().getPosition() + roads.get(i).getVehicle().getSpeed()
                        ...
                        roads.get(i + 1).createVehicle("Car", (roads.get(i).getVehicle().getPosition() + roads.get(i).getVehicle().getSpeed()) % roads.get(i).getLength()); //Creates car on next road
                        roads.get(i + 1).getVehicle().setSpeed(roads.get(i).getVehicle().getSpeed()); //Sets new cars speed
                        roads.get(i).destroyVehicle(roads.get(i).getVehicle().getPosition()); //Removes car from road
                    }
                    if (roads.get(i).getVehicle() != null) { //checks for vehicle
                        if (roads.get(i).getVehicle().getPosition() + roads.get(i).getVehicle().getSpeed() > (
                                roads.get(i).getTrafficLight().getPosition() -
                                        roads.get(i).getVehicle().getSpeed() / roads.get(i).getVehicle().getDeceleration()) && roads.get(i).getTrafficLight().status)
